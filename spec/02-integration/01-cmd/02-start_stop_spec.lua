...
describe("kong start/stop", function()
  ...
  describe("Database", function()
    it("starts with IP address with Cassandra", function()
      assert(helpers.kong_exec("start --conf "..helpers.test_conf_path, {cassandra_contact_points="127.0.0.1", database="cassandra"}))
    end)
    it("starts with IP address with Postgres", function()
      assert(helpers.kong_exec("start --conf "..helpers.test_conf_path, {cassandra_contact_points="127.0.0.1", database="postgres"}))
    end)
    it("starts with hostnames with Cassandra", function()
      assert(helpers.kong_exec("start --conf "..helpers.test_conf_path, {cassandra_contact_points="localhost", database="cassandra"}))
    end)
    it("starts with hostnames with Postgres", function()
      assert(helpers.kong_exec("start --trace --conf "..helpers.test_conf_path, {pg_host="localhost", database="postgres"}))
    end)
  end)
